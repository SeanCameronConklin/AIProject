...
Rules = "This is a simple soduku solver.\nEach red box accepts user input, each grey box is a starter value.\n"\
            "The goal of the game is to fill each row, column, and 3X3 box with numbers 1-9.\n"\
                ...
                while x in openSquares:
                    ...
                    while valueT <= 9:
                        if valueT in columnNum and valueT in rowNum and valueT in boxNum:
                            ...
                            break
                        elif valueT not in columnNum:
                            possible = False
                            valueT+= 1
                        elif valueT not in rowNum:
                            possible = False
                            valueT += 1
                        elif valueT not in boxNum:
                            possible = False
                            valueT += 1

            gameState = systemMoves

            if openSquares is None:
                possible = False
            elif openSquares:
                done = True
                return gameState
...
class sudokuGUI(tkinter.Tk):
    ...
    def initialize(self):
        ...
        messageB = tkinter.Text(self, height=13, width=50, bg="grey")
